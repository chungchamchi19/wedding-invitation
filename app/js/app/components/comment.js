...
export const comment = (() => {
  ...
  const showMore = (anchor, uuid) => {
    ...
    util.safeInnerHTML(
      ...
      util.convertMarkdownToHTML(
        util.escapeHtml(isCollapsed ? original : original.slice(0, card.maxCommentLength) + "...")
      )
    );
  ...
  const fetchTracker = async (c) => {
    ...
    const setResult = (result) => {
      ...
      util.safeInnerHTML(
        commentIp,
        `<i class="fa-solid fa-location-dot me-1"></i>${util.escapeHtml(c.ip)} <strong>${util.escapeHtml(
          result
        )}</strong>`
      );
    };
  ...
  const send = async (button) => {
    ...
    if (id) {
      ...
      document
        ...
        .insertAdjacentHTML("beforeend", await card.renderContentSingle(response.data));
      ...
      const readMoreElement = document
        ...
        .createContextualFragment(
          card.renderReadMore(id, anchorTag ? anchorTag.getAttribute("data-uuids").split(",").concat(uuids) : uuids)
        );
  ...
  const cancel = async (button, id) => {
    ...
    if (
      form.value.length === 0 ||
      (util.base64Encode(form.value) === form.getAttribute("data-original") && isChecklist === isPresent) ||
  ...
  const edit = async (button, is_parent) => {
    ...
    document
      ...
      .insertAdjacentElement("afterend", card.renderEdit(id, isChecklist, isParent, !!gifImage));
